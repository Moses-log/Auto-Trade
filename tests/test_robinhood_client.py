...
# ── get_portfolio_pct_change_async ────────────────────────────────────────────

@pytest.mark.asyncio
async def test_get_portfolio_pct_change_returns_none_when_unavailable():
    from app.trading.robinhood_client import RobinhoodClient
    client = RobinhoodClient()
    client.available = False

    result = await client.get_portfolio_pct_change_async("day", "5minute")

    assert result is None


@pytest.mark.asyncio
async def test_get_portfolio_pct_change_computes_pct_from_adjusted_equity():
    from app.trading.robinhood_client import RobinhoodClient
    client = RobinhoodClient()
    client.available = True
    ...
    historicals = {
        "equity_historicals": [
            {"begins_at": "2026-06-12T09:30:00Z", "adjusted_open_equity": "10000.00", "adjusted_close_equity": "10050.00"},
            {"begins_at": "2026-06-12T15:30:00Z", "adjusted_open_equity": "10050.00", "adjusted_close_equity": "10100.00"},
        ]
    }
    with patch("robin_stocks.robinhood.get_historical_portfolio", return_value=historicals):
        result = await client.get_portfolio_pct_change_async("day", "5minute")

    assert result == pytest.approx(1.0)  # (10100 - 10000) / 10000 * 100


@pytest.mark.asyncio
async def test_get_portfolio_pct_change_falls_back_to_unadjusted_equity():
    from app.trading.robinhood_client import RobinhoodClient
    client = RobinhoodClient()
    client.available = True
    ...
    historicals = {
        "equity_historicals": [
            {"begins_at": "2026-06-12T09:30:00Z", "open_equity": "5000.00", "close_equity": "4900.00"},
        ]
    }
    with patch("robin_stocks.robinhood.get_historical_portfolio", return_value=historicals):
        result = await client.get_portfolio_pct_change_async("day", "5minute")

    assert result == pytest.approx(-2.0)  # (4900 - 5000) / 5000 * 100


@pytest.mark.asyncio
async def test_get_portfolio_pct_change_returns_none_on_empty_historicals():
    from app.trading.robinhood_client import RobinhoodClient
    client = RobinhoodClient()
    client.available = True
...
@pytest.mark.asyncio
async def test_get_portfolio_pct_change_returns_none_on_exception():
    from app.trading.robinhood_client import RobinhoodClient
    client = RobinhoodClient()
    client.available = True
...
@pytest.mark.asyncio
async def test_get_portfolio_pct_change_ytd_filters_by_since():
    from app.trading.robinhood_client import RobinhoodClient
    ...
    historicals = {
        "equity_historicals": [
            {"begins_at": "2025-12-15T00:00:00Z", "adjusted_open_equity": "8000.00", "adjusted_close_equity": "8500.00"},
            {"begins_at": "2026-01-05T00:00:00Z", "adjusted_open_equity": "9000.00", "adjusted_close_equity": "9090.00"},
            {"begins_at": "2026-06-01T00:00:00Z", "adjusted_open_equity": "9500.00", "adjusted_close_equity": "9690.00"},
        ]
    }
    since = datetime(2026, 1, 1, tzinfo=timezone.utc)
    with patch("robin_stocks.robinhood.get_historical_portfolio", return_value=historicals):
        result = await client.get_portfolio_pct_change_async("year", "day", since=since)

    # Only the 2026 entries count: (9690 - 9000) / 9000 * 100
    assert result == pytest.approx((9690 - 9000) / 9000 * 100)
