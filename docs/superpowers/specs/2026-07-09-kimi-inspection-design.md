...
5. Prompt gives the model real freedom to `SELL`/`TRIM`/`DOUBLE_DOWN` (revised 2026-09: the original default-to-`HOLD` bias was removed because Inspection almost never acted). It acts on any clear, nameable reason: earnings surprise, guidance change, major company-specific news, macro shock tied to the name, meaningful technical breakdown, overweight size (>~20% of book), stretched or cheap valuation vs. thesis, thesis drift/strengthening, or strong/weak momentum — but not single-day price noise. Code additionally force-trims any holding above the 25% cap back to 25% (`enforce_position_cap`), even if the model said `HOLD`. Same position-sizing constraints as the monthly rebalance apply (25% cap, no sector above 50%, resolved-bear-case required above 10% for any `DOUBLE_DOWN`).
