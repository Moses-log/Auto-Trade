...
| **Fix (part 1 — source)** | New `pnl._deposit_events()`: the **investor ledger is the source of truth** (all deposits go through `/deposit`), with the Alpaca API as a fallback only when the ledger is empty. Wired into all 7 deposit-adjustment call sites (daily, weekly, monthly, YTD, 1-year, all-time, inception). Applies to **every investor's deposits**, any size, past and future. |
| **Fix (part 2 — alignment)** | Sourcing alone left a *transient* spike at each deposit: a `/deposit` date (`date.today()`) lags the Alpaca equity bar where the cash actually lands, so subtracting on the recorded bar leaves a one-bar up-spike. `deposit_adjusted_equity` now **aligns each deposit to its equity jump** — within ±5 days of the recorded date it picks the positive bar-over-bar jump closest in size to the deposit amount and subtracts from there. The fallback stays inside the window, so a deposit predating a report's series (already in the baseline) is never stripped. |
| **Tests** | Reproduce the two-spike bug (RED → GREEN): late-recorded, two-late-deposits, and early-recorded deposits all flatten to a smooth curve; an out-of-window deposit is left untouched; and a companion test documents that the old API-only path leaves a sub-20% spike. |
